                     ...
                     const lv_draw_rect_dsc_t * draw_dsc)
{
    ...
    vg_lite_path_t path;
    // lv_color32_t colMix;
    // lv_opa_t opa = draw_dsc->bg_opa;

    // colMix.ch.red = ((uint16_t)draw_dsc->bg_color.ch.red * opa) >> 8; /*Pre-multiply color*/
    // colMix.ch.green = ((uint16_t)draw_dsc->bg_color.ch.green * opa) >> 8;
    // colMix.ch.blue = ((uint16_t)draw_dsc->bg_color.ch.blue * opa) >> 8;
    // colMix.ch.alpha = opa;
    ...
    if(vg_lite_init_path(&path, VG_LITE_S16, VG_LITE_HIGH, path_size, path_data, clip_area->x1, clip_area->y1, clip_area->x2, clip_area->y2) == VG_LITE_SUCCESS) {
        if (vg_lite_upload_path(&path) == VG_LITE_SUCCESS) {
            vg_lite_buffer_t * rt_vgbuf = vglite_get_buffer(NULL);
            vg_lite_matrix_t matrix;
            vg_lite_identity(&matrix);
            vg_lite_error_t err = vg_lite_draw(rt_vgbuf, &path, VG_LITE_FILL_EVEN_ODD, &matrix, VG_LITE_BLEND_SRC_OVER, draw_dsc->bg_color.full/*colMix.full*/);
            err |= vg_lite_finish();
